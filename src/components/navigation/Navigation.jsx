...
import { logoIcon, linkedinIcon, emailIcon, twitterIcon } from "../../assets/icons";
  ...
  return (
    <nav className={styles.nav}>
      <div className={`flex ${styles.nav_container}`}>
          ...
          />
          <Link to={resumeLink} className='semi-bold t-large' target="_blank">My Resume</Link>
          <div className={`flex gap-2 ${styles.icon_container}`}>
            <Link to={`mailto:${emailLink}`}><img src={emailIcon} alt="email-icon" /></Link>
            <Link to={linkedinLink} target="_blank"><img src={linkedinIcon} alt="linkedin-icon" /></Link>
            <Link to={twitterLink} target="_blank"><img src={twitterIcon} alt="twitter-icon" /></Link>
          </div>
      </div>
    </nav>
  );
